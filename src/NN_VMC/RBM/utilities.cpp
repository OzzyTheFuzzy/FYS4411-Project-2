...
std::string modeToString(Mode m) {return (m == Mode::Importance) ? "is" : "bf";}
...
// ---------- scale helpers ----------
void scale1D(std::vector<double>& A, double factor) {
    for (double& x : A) {
        x *= factor;
    }
}
void scale2D(std::vector<std::vector<double>>& A, double factor) {
    for (auto& row : A) {
        for (double& x : row) {
            x *= factor;
        }
...
void scale3D(std::vector<std::vector<std::vector<double>>>& A, double factor) {
    for (auto& mat : A) {
        for (auto& row : mat) {
            for (double& x : row) {
                x *= factor;
...
RunResult runVMC(
    ...
    if (useInteraction) {
        hamiltonian = std::make_unique<RBMInteractingTrap>(gamma, 1.0);
    } else {
